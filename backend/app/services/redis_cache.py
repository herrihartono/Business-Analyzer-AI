...
- OpenAI responses (keyed by prompt hash)
"""
...
# ── OpenAI response cache (deduplication of identical prompts) ───

async def get_cached_openai(system_prompt: str, user_prompt: str) -> str | None:
    key = f"openai:{_hash_key(system_prompt, user_prompt)}"
    result = await _get(key)
    return result.get("response") if result else None


async def set_cached_openai(system_prompt: str, user_prompt: str, response: str) -> None:
    key = f"openai:{_hash_key(system_prompt, user_prompt)}"
    await _set(key, {"response": response}, TTL_OPENAI)
