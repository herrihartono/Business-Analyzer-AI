...
async def flush_cache():
    """Flush all SmartBiz cache entries (analysis, chat, llm, dashboard)."""
    if not _pool:
        return {"flushed": 0, "message": "Redis not connected"}

    total = 0
    for prefix in ("analysis:*", "chat:*", "llm:*", "dashboard:*"):
...
async def flush_cache_by_prefix(prefix: str):
    """Flush cache entries by prefix: analysis, chat, llm, or dashboard."""
    allowed = {"analysis", "chat", "llm", "dashboard"}
